...
class AdvanceFilterComponent extends Component
{
    ...
    protected function createTemplateJson()
    {
        ...
        if (Arr::has($this->tender_no, ['from'])){
            array_push($holdArray,['queryCol'=>'tender_no' , 'queryOpr'=>'=' , 'queryVal'=>$this->tender_no['from']]);
        }
        if (Arr::has($this->tender_desc, ['from'])){
            array_push($holdArray,['queryCol'=>'tender_desc' , 'queryOpr'=>'LIKE' , 'queryVal'=>$this->tender_desc['from']]);
        }
       if (Arr::has($this->document_type, ['from'])){
            array_push($holdArray,['queryCol'=>'document_type' , 'queryOpr'=>'=' , 'queryVal'=>$this->document_type['from']]);
        }
        ...
        if (Arr::has($this->purchasing_organization, ['from'])){
             array_push($holdArray,['queryCol'=>'purchasing_organization' , 'queryOpr'=>'=' , 'queryVal'=>$this->purchasing_organization['from']]);
        }
        if (Arr::has($this->customer_no, ['from'])){
             array_push($holdArray,['queryCol'=>'customer_no' , 'queryOpr'=>'=' , 'queryVal'=>$this->customer_no['from']]);
        }
        if (Arr::has($this->nupco_delivery_date, ['from'])){
             array_push($holdArray,['queryCol'=>'nupco_delivery_date' , 'queryOpr'=>'=' , 'queryVal'=>$this->nupco_delivery_date['from']]);
        }
        if (Arr::has($this->po_created_on, ['from'])){
             array_push($holdArray,['queryCol'=>'po_created_on' , 'queryOpr'=>'=' , 'queryVal'=>$this->po_created_on['from']]);
        }
        if (Arr::has($this->generic_mat_code, ['from'])){
             array_push($holdArray,['queryCol'=>'generic_mat_code' , 'queryOpr'=>'=' , 'queryVal'=>$this->generic_mat_code['from']]);
        }
        if (Arr::has($this->vendor_code, ['from'])){
             array_push($holdArray,['queryCol'=>'vendor_code' , 'queryOpr'=>'=' , 'queryVal'=>$this->vendor_code['from']]);
        }
        if (Arr::has($this->storage_location, ['from'])){
             array_push($holdArray,['queryCol'=>'storage_location' , 'queryOpr'=>'=' , 'queryVal'=>$this->storage_location['from']]);
        }
        if (Arr::has($this->plant, ['from'])){
             array_push($holdArray,['queryryCol'=>'plant' , 'queryOpr'=>'=' , 'queryVal'=>$this->plant['from']]);
        }
        if (Arr::has($this->customer_name, ['from'])){
             array_push($holdArray,['queryCol'=>'customer_name' , 'queryOpr'=>'=' , 'queryVal'=>$this->customer_name['from']]);
        }
        if (Arr::has($this->supply_ratio, ['from'])){
             array_push($holdArray,['queryCol'=>'supply_ratio' , 'queryOpr'=>'=' , 'queryVal'=>$this->supply_ratio['from']]);
        }
        if (Arr::has($this->delivery_address, ['from'])){
             array_push($holdArray,['queryCol'=>'delivery_address' , 'queryOpr'=>'LIKE' , 'queryVal'=>$this->delivery_address['from']]);
        }
        if (Arr::has($this->mat_description, ['from'])){
             array_push($holdArray,['queryCol'=>'mat_description' , 'queryOpr'=>'=' , 'queryVal'=>$this->mat_description['from']]);
        }
        if (Arr::has($this->cust_gen_code, ['from'])){
             array_push($holdArray,['queryCol'=>'cust_gen_code' , 'queryOpr'=>'=' , 'queryVal'=>$this->cust_gen_code['from']]);
        }
        if (Arr::has($this->vendor_name_en, ['from'])){
             array_push($holdArray,['queryCol'=>'vendor_name_en' , 'queryOpr'=>'=' , 'queryVal'=>$this->vendor_name_en['from']]);
        }
        if(Arr::has($this->supplier_comment, ['from']) and $this->supplier_comment['from'] !='0' ){
            array_push($holdArray,['queryCol'=>'supplier_comment' , 'queryOpr'=>'=' , 'queryVal'=>$this->supplier_comment['from']]);
        }
        return $holdArray;
    }
    ...
    {
        return view('livewire.filters.advance-filter-component');
    }
}
